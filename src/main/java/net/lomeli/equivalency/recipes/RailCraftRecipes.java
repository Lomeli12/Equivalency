    ...
    public static void loadRecipes(ItemStack transmutationStone, String modName) {
        ...
        for (ItemStack steel : steelIngot) {
            if (ModLoaded.isModInstalled(ModVars.IC2_ID) && ModVars.steelTransmute) {
                TransmutationHelper.addRecipe(new ItemStack(steel.getItem(), 2, steel.getItemDamage()), transmutationStone, new Object[] { IC2Recipes.machineBlock,
                        IC2Recipes.machineBlock, IC2Recipes.machineBlock });

                TransmutationHelper.addRecipe(new ItemStack(IC2Recipes.machineBlock.getItem(), 3, IC2Recipes.machineBlock.getItemDamage()), transmutationStone, new Object[] {
                        steel, steel });
            }
        }
        for (ItemStack coke : coalCoke) {
            // 4 Coal/CharCoal -> 1 Coal Coke
            TransmutationHelper.addRecipe(coke, transmutationStone, new Object[] { (new ItemStack(Item.coal, 1, 1)), (new ItemStack(Item.coal, 1, 1)),
                    (new ItemStack(Item.coal, 1, 1)), (new ItemStack(Item.coal, 1, 1)) });
            // 1 Coal Coke -> 4 Coal/CharCoal
            TransmutationHelper.addRecipe(new ItemStack(Item.coal, 4, 1), transmutationStone, new Object[] { coke });
        }
    }
}
