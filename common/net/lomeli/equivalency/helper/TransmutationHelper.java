...
public class TransmutationHelper {
    ...
    public static void addStones() {
        transmutationStones.add(new ItemStack(ItemUtil.getItem("miniumStone",
                ITEM_LOC).getItem(), 1, WILDCARD));
        transmutationStones.add(new ItemStack(ItemUtil.getItem("philStone",
                ITEM_LOC).getItem(), 1, WILDCARD));
    }

    public static void addRecipe(ItemStack output,
            ItemStack transmutationStone, Object... input) {
        Object[] inputs = new Object[input.length + 1];
        System.arraycopy(input, 0, inputs, 0, input.length);
        inputs[input.length] = new ItemStack(transmutationStone.getItem(), 1,
                WILDCARD);
    ...
    public static void addRecipe(Item output, ItemStack transmutationStone,
            Object... input) {
        addRecipe(new ItemStack(output), transmutationStone, input);
    }

    public static void addRecipe(Block output, ItemStack transmutationStone,
            Object... input) {
        addRecipe(new ItemStack(output), transmutationStone, input);
    }

    // Copied from EE3 for easier recipe registering.
    public static void addSmeltingRecipe(ItemStack input, ItemStack stone,
            ItemStack fuel) {
        ...
        if (result.stackSize * 7 <= result.getItem().getItemStackLimit())
            GameRegistry.addShapelessRecipe(new ItemStack(result.getItem(),
                    result.stackSize * 7, result.getItemDamage()), list);
        else
            GameRegistry
                    .addShapelessRecipe(
                            new ItemStack(result.getItem(), result.getItem()
                                    .getItemStackLimit(), result
                                    .getItemDamage()), list);
    }
